...
static t_llist	*strsizeexc(t_cchar *args, t_crd *crd, t_cchar *end, t_hash *hst)
{
	t_cchar	*tmp;
	t_llist	*llst;
	...
	while (*args && !(ft_strlcmp(args, end) && *(args - 1) != ESCCH))
	{
		...
		{
			crd->size += ft_strlen((char *)(llistadd_back(&llst,
	...
	return (llistadd_front(&llst, llistnewnode((void *)crtintdt(args - tmp))));
}

static int	setresstr(char	*res, t_cchar *args, t_crd *crd, t_llist *llst)
{
	int		i;
	t_cchar	*tmp;
	...
	tmp = args;
	i = 0;
    while ((int)(args - tmp) < crd->size)
    {
		if (llst && checkvarch(args, VARCH))
		{
			i = ft_strcpy(res + i, (t_cchar *)llst->data) - res; 
			while (*args && *args != SPCCH && (int)(args - tmp) < crd->size)
				++args;
			llst = llst->next;
			continue ;
		}
...
char	*dqtshandler(t_cchar *str, t_crd *crd, t_cchar *end, t_hash *hst)
{
	...
	llst = strsizeexc(str, crd, end, hst);
	if (!llst)
	{
		crd->size = -1;
	...
	if (!res)
		return (ft_perror(MALLOCERROR));
	crd->size = *(int *)(llst->data);
	crd->i += setresstr(res, str, crd, llst->next);
	llistclear(&llst, free);
	return (res);
}
